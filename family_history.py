...
def main():
    ...
    print_marriage(marriage_dict, people_dict)
...
def count_genders(people_dict):
    ...
    for key, value in people_dict.items():
        gen = value[GENDER_INDEX]
        if gen == "M":
            males += 1

    number_people = len(people_dict)
    females = number_people - males
...
def print_marriage(marriage_dict, people_dict):
    ...
    for marriage_key, marriage_list in marriage_dict.items():
        husband_key = marriage_list[HUSBAND_KEY_INDEX]
        wife_key = marriage_list[WIFE_KEY_INDEX]
        wedding_key = marriage_list[WEDDING_YEAR_INDEX]
        ...
        husband_age = wedding_key - husband_birth
